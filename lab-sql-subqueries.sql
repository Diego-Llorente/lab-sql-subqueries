...
WHERE film_id IN (
	...
	WHERE actor_id = (
		SELECT actor_id
		FROM(
			SELECT actor_id, COUNT(film_id) AS times_starred
			FROM film_actor
			GROUP BY actor_id
			ORDER BY times_starred DESC
...
#	GROUP BY actor_id
#	ORDER BY times_starred DESC
#	LIMIT 1) AS most_prolific_actor; #this get's me the most prolific actor_id
...
WHERE film_id IN (
	...
	WHERE inventory_id IN (
		...
		WHERE customer_id = (
			SELECT customer_id 
			FROM (
			SELECT customer_id, SUM(amount) AS total_money_spent
			FROM payment
...
#SELECT customer_id 
#FROM (SELECT customer_id, SUM(amount) AS total_money_spent
#FROM payment
#GROUP BY customer_id
#ORDER BY total_money_spent DESC
#LIMIT 1) as most_profitable_customer;
...
FROM (
	...
	FROM payment
	GROUP BY customer_id) AS client_total_amount
